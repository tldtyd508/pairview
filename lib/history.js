function parseHistoryFilters(params) {
  const get = (key) => {
...
function isOnOrAfter(happenedOn, from) {
  return !from || happenedOn >= from;
}

function isOnOrBefore(happenedOn, to) {
...
function experienceTextFields(experience) {
...
function reviewScoreOf(experience, userId) {
  return experience.reviews.find((review) => review.user_id === userId)?.score ?? null;
}

function combinedScoreOf(experience, currentUserId, partnerUserId) {
...
function filterAndSortExperiences(experiences, filters, currentUserId, partnerUserId) {
  ...
  const filtered = experiences.filter((experience) => {
    ...
    const myScore = reviewScoreOf(experience, currentUserId);
    const partnerScore = partnerUserId ? reviewScoreOf(experience, partnerUserId) : null;
    const bestScore =
  ...
  filtered.sort((left, right) => {
    ...
    if (filters.sort === "oldest") {
      return (
        left.happened_on.localeCompare(right.happened_on) ||
...
function completionLabel(reviewCount) {
